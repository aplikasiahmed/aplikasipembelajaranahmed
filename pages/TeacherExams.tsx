...
const TeacherExams: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 animate-fadeIn pb-20 px-1 md:px-0">
      ...
      {/* HEADER CARD */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm z-10 relative">
        <div>
        ...
        <button 
          onClick={toggleForm}
          className={`px-5 py-3 rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 text-xs font-black uppercase tracking-wider ${showForm ? 'bg-red-50 text-red-600 shadow-red-100' : 'bg-emerald-600 text-white shadow-emerald-200'}`}
        >
