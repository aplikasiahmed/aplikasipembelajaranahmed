...
import { Search, Play, Timer, CheckCircle, AlertTriangle, ArrowRight, HelpCircle } from 'lucide-react';
...
const PublicExam: React.FC = () => {
  ...
  // Exam Execution
  const [answers, setAnswers] = useState<Record<string, string>>({}); // qId -> answerIndex
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (s) {
        setStudent(s);
        // Load Exams for this student's grade
        const gradeChar = s.kelas.charAt(0); // '7', '8', '9'
        const exams = await db.getActiveExamsByGrade(gradeChar);
        setActiveExams(exams);
        setStep('list');
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: `Halo, ${s.namalengkap}`, showConfirmButton: false, timer: 1500 });
      } else {
  ...
  const startExam = async (exam: Exam) => {
    ...
    const res = await Swal.fire({
      title: 'Mulai Ujian?',
      html: `Waktu pengerjaan: <b>${exam.duration} Menit</b>.<br>Pastikan koneksi internet lancar.`,
      icon: 'info',
    ...
    if (res.isConfirmed) {
      ...
      setQuestions(q);
      setAnswers({});
      setTimeLeft(exam.duration * 60); // Menit ke Detik
      setStep('exam');
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto min-h-[50vh] flex flex-col justify-center animate-fadeIn px-4">
        <div className="text-center space-y-2 mb-6">
          ...
          <p className="text-xs text-slate-500">Masukkan NIS untuk mengerjakan soal</p>
        </div>
        
        <form onSubmit={handleLogin} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 space-y-3">
          <div>
            ...
            <div className="relative">
              ...
              <input 
                type="text" 
                inputMode="numeric"
                // UPDATE: Padding dikurangi jadi py-2.5 (lebih ramping)
                className="w-full bg-slate-50 pl-11 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold outline-none focus:border-emerald-500 focus:bg-white transition-all placeholder:font-normal text-slate-800"
          ...
          <button type="submit" className="w-full bg-emerald-600 text-white py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-95 transition-all">
             Masuk Ujian
          </button>
        </form>
      </div>
  ...
  if (step === 'list') {
    return (
      <div className="max-w-2xl mx-auto space-y-4 animate-fadeIn pb-24 px-1 md:px-0 pt-4">
        ...
        <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight ml-1">Daftar Soal</h2>
        
        <div className="space-y-3">
          {activeExams.length === 0 ? (
             <div className="p-8 text-center bg-white rounded-2xl border border-dashed border-slate-200">
               <AlertTriangle className="mx-auto text-slate-200 mb-2" size={32} />
               <p className="text-slate-400 text-xs font-bold">Tidak ada soal yang harus dikerjakan</p>
             </div>
          ) : (
               ...
               <div key={exam.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:border-emerald-300 transition-all group">
                 <div className="flex justify-between items-start mb-4">
                    <div>
                       ...
                       <p className="text-[10px] text-slate-400 mt-1">Semester {exam.semester} • {exam.duration} Menit</p>
                    </div>
  ...
  if (step === 'exam' && selectedExam) {
    return (
      <div className="max-w-3xl mx-auto min-h-screen pb-20 animate-fadeIn bg-slate-50 relative">
        {/* Sticky Timer Header - Modernized */}
        ...
        <div className="space-y-6 px-3 md:px-0">
          {questions.map((q, idx) => (
            // UPDATE: Desain Soal Card "Tidak Flat"
               ...
               <div className="relative z-10">
                   <div className="flex gap-4 mb-4">
                      ...
                      <div className="flex-1 pt-1">
                        {/* UPDATE: Render Gambar Soal */}
                        {q.image_url && (
                            <div className="mb-4 rounded-xl overflow-hidden border border-slate-100 shadow-sm max-w-md">
                                <img src={q.image_url} alt="Soal" className="w-full h-auto object-contain bg-slate-50" />
                            </div>
                        )}
                     ...
                     {q.options?.map((opt, optIdx) => {
                       ...
                       return (
                         <button
                           ...
                           // UPDATE: Desain Tombol Jawaban Lebih Timbul & Interaktif
  ...
  if (step === 'result') {
    return (
      <div className="max-w-md mx-auto min-h-[60vh] flex flex-col items-center justify-center px-4 animate-slideUp text-center">
        ...
        <p className="text-sm text-slate-500 mb-8 max-w-xs mx-auto">Terima kasih telah mengerjakan ujian dengan jujur.</p>
